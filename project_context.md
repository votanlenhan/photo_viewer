...
*   **Dọn dẹp code:** Đã xóa các action API cũ/sai (`list_sub_items`, `verify_password`) và hàm helper cũ (`sanitize_subdir`).
*   **Sửa lỗi & Cải thiện UX:**
    ...
    *   Đã thêm phản hồi tiến trình (spinner, thông báo, nút hủy) khi tải file ZIP.
    *   Đã thêm hiệu ứng làm mờ nền (blur) khi hiển thị lớp phủ tiến trình ZIP và lớp phủ nhập mật khẩu.
